...
void height_data_deal(float dt)
{
    ...
    // 2.1 哨兵值检测：dl1b_distance_mm=0或>4000 表示传感器无有效数据
    if (dl1b_distance_mm > 4000 || dl1b_distance_mm == 0) {
        laser_was_invalid = 1;
    ...
    // 2.3 姿态角补偿：斜距投影为对地高度
    // cos_factor = cos(pitch)×cos(roll)，投影系数
    // 滤波避免姿态变化过快时IMU与TOF时序不匹配导致高度跳变
    static PT1Filter_t filter_cos;
    static uint8_t cos_filter_init = 0;
    if(!cos_filter_init) {
        PT1Filter_InitWithFreq(&filter_cos, 30.0f, 200);  // 30Hz滤波
        cos_filter_init = 1;
    }
    ...
    const float VZ_NORM = 0.30f;   // 悬停噪声约 0.05，放宽到 0.15
    // 意义：悬停时激光差分速度噪声通常在 0.02~0.05m/s 以内
    //       低于此值认为是正常数据，完全信任激光
    const float VZ_MAX  = 1.00f;   // 衰减终点 1.0
...
// printf("TOF: raw=%uum, height=%.2fm, laser_quality=%.2f\r\n",
//             dl1b_distance_mm, alt.height, alt.laser_quality);
