...
static uint8 find_v_from_hough(const HoughLine *lines, uint8 line_cnt, Blob *b)
{
    for (uint8 i = 0; i < line_cnt; i++)
    {
        ...
        {
            // 要求一条正斜率、一条负斜率
            if ((lines[i].theta_deg > 0 && lines[j].theta_deg > 0) ||
                (lines[i].theta_deg < 0 && lines[j].theta_deg < 0))
                continue;
            ...
            // ★★★ 顶点到 blob 质心的距离约束: 防止飞点
            // 霍夫检测到的直线交点 (顶点) 应靠近当前 blob, 距离过远说明是杂散直线
            float dvx = ax - (float)b->cx;
            float dvy = ay - (float)b->cy;
            float v_dist2 = dvx * dvx + dvy * dvy;
...
static void compute_car_marker_feature(Blob *b, Point *pts, uint32 count)
{
    ...
    {
        // ===== BFS 几何法失败 → 霍夫变换后备 =====
        // 常见失败场景: V 形被遮挡, 分裂为 √ 形 + 小条
        // 霍夫可以在图像级跨碎片找到两条直线臂
        // ROI 取 blob bbox 外扩 HOUGH_ROI_MARGIN px, 以包含相邻碎片
        ...
        // ★★★ 边界保护: 如果 blob 太靠近图像外框, 跳过霍夫后备
        // 因为边缘处的 ROI 被截断, 容易误检屏幕边框为 V 形
        #define CAR_MARK_EDGE_MARGIN  (HOUGH_ROI_MARGIN + 8)
        ...
        {
#if DEBUG_SCORES
            printf("HOUGH: rejected by edge margin (%d)\n", (int)CAR_MARK_EDGE_MARGIN);
#endif
        ...
        // 动态生成角度列表: ±[HOUGH_ANGLE_MIN, HOUGH_ANGLE_MAX], 步长 HOUGH_ANGLE_STEP
        int8 hough_angles[48]; // 最大 48 个角度
        uint8 idx = 0;
        ...
        uint8 hough_cnt = hough_transform_roi(
            b->cx, b->cy,
...
static void compute_raw_scores(void)
{
    for(uint8 i = 0; i < blob_num; i++)
    {
        ...
        // 长条惩罚：ord>30 时信标分快速衰减，ord≥60 归零
        // 真正信标灯接近圆形，长条(如 V 形碎片)不可能是信标
        if(ord > 30)
        {
            int32 factor = (ord >= 60) ? 0 : (60 - ord);  // 30→30, 60→0 (原 70→0)
            bs = bs * factor / 30;                        // 衰减更陡 (原 /40)
        }
        ...
        // ---- 长条形保底车标分 ----
        // 场景: V 形被遮挡, BFS/霍夫均失败, marker_* 特征为空
        // 此时 raw_marker_score == 0, 但 blob 明显细长不可能是信标
        // 给予一个保底分, 让软判决至少能对抗信标分
        {
            if(blobs[i].raw_marker_score == 0 && ord > 40 && blobs[i].area >= CAR_MARK_MIN_AREA)
            {
                // 越细长保底分越高
                uint16 bonus = FRAG_MARKER_BONUS;  // 基础 20
                if(ord >= 80) bonus += 10;         // 特别细长的再加 10
...
static void pair_v_fragments(void)
{
    ...
    #define FRAG_MIN_ELONGATION 180    // 1.8x 长宽比 (原250, 降低以匹配短碎片)
