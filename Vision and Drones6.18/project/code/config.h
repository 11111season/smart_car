...
#define POSITION_HOLD               (1)     // 位置保持
#define POSITION_TARGET_ZERO        (1)     // 1=定点悬停(只用速度环), 0=视觉追踪小车
...
// =============================================================================
#define ANG_RATE_FF_GAIN            (0.05f)  // 角速度前馈增益 (0=关闭, 0.3=保守)         
...
#define VELH_KP                     (450.0f)  // 垂直速度内环 P
#define VELH_KI                     (20.0f)  // 垂直速度内环 I
...
#define POS_KP                      (0.02f)  // 位置外环 P
#define POS_KI                      (0.000f) // 位置外环 I
#define POS_KD                      (0.00f)  // 位置外环 D

// 内环(水平速度->角度): 将速度误差转换为期望角度
#define POS_VELX_KP                 (8.0f) // X方向速度内环 P (前后→Pitch)，原7.0
#define POS_VELY_KP                 (8.0f) // Y方向速度内环 P (左右→Roll)，原6.0
#define POS_VEL_KI                  (0.5f)  // 水平速度内环 I
#define POS_VEL_KD                  (0.0f)  // 水平速度内环 D  

// ============s=================================================================
// 内环角速度PID参数 (PD控制)
// =============================================================================
#define VELX_KP                     (6.5f)  // Roll角速度环 P //6.5
#define VELX_KI                     (2.6f)  // Roll角速度环 I  // 2.6
#define VELX_KD                    (0.6f)   // Roll角速度环 D (阻尼抑制P震荡) // 0.6

#define VELY_KP                     (6.5f)  // Pitch角速度环 P  
...
// 起飞阶段控制配置
// =============================================================================
#define TAKEOFF_TARGET_HEIGHT       (1.2f)  // 起飞目标高度 (m)
