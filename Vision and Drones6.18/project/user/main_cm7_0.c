...
int main(void)
{
    ...
    // ========== VL53L5CX TOF 传感器测试 ==========
    // 注意：VL53L5CX 与 DL1B 共用 I2C 总线且地址相同(0x29)，
    // 若冲突需注释掉 TOF.c 中 dl1b_init() 的调用
    {
        uint8_t tof_ret = vl53l5cx_init();
        printf("VL53L5CX init: %s\r\n", (0 == tof_ret) ? "OK" : "FAIL");
        if (0 == tof_ret)
        {
    ...
    while(true)  
    {
       // 每 50 次循环读取一次 VL53L5CX 距离
       if (++tof_cnt >= 50) {
           tof_cnt = 0;
           vl53l5cx_get_distance();
           if (vl53l5cx_finsh_flag) {
               printf("VL53L5CX zone0=%umm(st=%u) zone1=%umm(st=%u) zone2=%umm(st=%u) zone3=%umm(st=%u)\r\n",
                      vl53l5cx_zone_result.distance_mm[0], vl53l5cx_zone_result.status[0],
                      vl53l5cx_zone_result.distance_mm[1], vl53l5cx_zone_result.status[1],
                      vl53l5cx_zone_result.distance_mm[2], vl53l5cx_zone_result.status[2],
                      vl53l5cx_zone_result.distance_mm[3], vl53l5cx_zone_result.status[3]);
           }
       }
