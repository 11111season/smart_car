 ...
 * @name       : PID_SetTarget
 * @date       : 2026-03-10
 * @function   : PID的设置目标值函数
 * @parameters : 传入PID结构体，目标值
 * @retvalue   : 无
 * @note       : 无
******************************************************************************/
 ...
 * @date       : 2026-03-10
 * @function   : PID的计算函数
 * @parameters : 传入PID结构体，反馈值
 * @retvalue   : 输出PID控制量（pwm）
 * @note       : 无
******************************************************************************/
float PID_Calculate(PID *pid, float FeedBack)
{
    ...
    // 3. 计算各项输出
    float COp = 0.0f;  // 比例项输出
    float COi = 0.0f;  // 积分项输出
    float COd = 0.0f;  // 微分项输出
    ...
    // 6. 更新历史状态
    pid->err_k_1 = err;          // 保存当前误差，下次作为err[k-1]
