...
{
    float avg = (motor_L1.encoder_speed + motor_L2.encoder_speed +
                 motor_R1.encoder_speed + motor_R2.encoder_speed) * 0.25f;
    float dp = enc2m((int32_t)fabsf(avg));
    if (avg < 0.0f) dp = -dp;

    float yaw_rad = My_Imu660ra_GetYaw() * (PI / 180.0f);
    g_pos_x += dp * cosf(yaw_rad);
    g_pos_y += dp * sinf(yaw_rad);
}
