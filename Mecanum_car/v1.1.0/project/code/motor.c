...
// 角度环与平衡环PID实例
PID angle_pid_yaw;      // 偏航角(Yaw)控制 (外环)
PID angle_pid_gyro;     // 角速度环 (内环, 陀螺仪反馈)
PID angle_pid_pitch;    // 俯仰角(Pitch)平衡控制
PID angle_pid_roll;     // 横滚角(Roll)平衡控制
...
uint8_t balance_mode = 0;              // 0: 普通模式, 1: 自平衡模式
...
{
    // 角度外环: 纯P, Kp=3.0 (角度误差→目标角速度)
    PID_Init(&angle_pid_yaw, 12.0f, 0.0f, 0.01f);
    PID_SetLimit(&angle_pid_yaw, 100.0f, -100.0f);
    PID_SetIntegralLimit(&angle_pid_yaw, 20.0f);
    PID_Enable(&angle_pid_yaw, 0);

    // 角速度内环: PI, Kp=2.0 Ki=0.3 (跟踪目标角速度, 陀螺仪无延迟反馈)
    PID_Init(&angle_pid_gyro, 0.3f, 0.0f, 0.001f);//0.2 0 0
    PID_SetLimit(&angle_pid_gyro, 100.0f, -100.0f);
    PID_SetIntegralLimit(&angle_pid_gyro, 50.0f);
    PID_Enable(&angle_pid_gyro, 0);
}

//-------------------------------------------------------------------------------------------------------------------
// 函数名称       Balance_Init
// 函数描述       自平衡PD控制器初始化 (仅用Pitch/Roll的比例+微分, 不积分)
// 调用位置       Motor_Init() 内部调用
//-------------------------------------------------------------------------------------------------------------------
void Balance_Init(void)
{
    // Pitch轴 (前后倾斜 → X方向速度补偿)
    PID_Init(&angle_pid_pitch, 0.02f, 0.0f, 0.0f);
    PID_SetLimit(&angle_pid_pitch, 0.2f, -0.2f);
    PID_SetIntegralLimit(&angle_pid_pitch, 0.0f);       // 平衡环不使用积分
    PID_Enable(&angle_pid_pitch, 0);

    // Roll轴 (左右倾斜 → Y方向速度补偿)
    PID_Init(&angle_pid_roll, 0.02f, 0.0f, 0.0f);
    PID_SetLimit(&angle_pid_roll, 0.2f, -0.2f);
    PID_SetIntegralLimit(&angle_pid_roll, 0.0f);
    PID_Enable(&angle_pid_roll, 0);
}
...
void Motor_Init(void)
{
    ...
    // ---- 四个电机速度PID初始化 (前馈+PID反馈架构) ----
    // Kp=1.0, 纯P控制, 前馈负责主力输出, PID做微调
    PID_Init(&motor_L1.pid, 4.5f, 0.3f, 0.05f);
    ...
    PID_Init(&motor_L2.pid, 4.6f, 0.35f, 0.05f);
    ...
    PID_Init(&motor_R1.pid, 4.8f, 0.35f, 0.02f);
    ...
    PID_Init(&motor_R2.pid, 4.5f, 0.35f, 0.03f);
...
{
    PID_Init(&pos_pid_x, 0.025f, 0.0f, 0.0f);
    PID_SetLimit(&pos_pid_x, 0.15f, -0.15f);            // 输出限幅: ±0.15 m/s
    PID_SetIntegralLimit(&pos_pid_x, 10.0f);
    PID_Enable(&pos_pid_x, 1);

    PID_Init(&pos_pid_y, 0.025f, 0.0f, 0.0f);
    PID_SetLimit(&pos_pid_y, 0.15f, -0.15f);
...
void Motor_PID_Control_All(void)
{
    ...
    if (motor_L2.pid.Enable) {
        ...
        //feedforward = Feedforward_Calculate(target);
        //total_output = -(feedforward + feedback);    // 取反补偿电机线反接
        total_output = -(feedback);    // 取反补偿电机线反接
        if (total_output > PWM_MAX_SAFE) total_output = PWM_MAX_SAFE;
        if (total_output < PWM_MIN_SAFE) total_output = PWM_MIN_SAFE;
...
// 函数描述       自平衡控制: 读取Pitch/Roll角度 → PD控制器 → 输出速度补偿 vx/vy
//               仅用PD (不积分), 防止平衡环积分饱和
//               输出限幅 ±0.15 m/s, 防止平衡环与速度环冲突
//-------------------------------------------------------------------------------------------------------------------
void Balance_Control(void)
{
    float pitch = My_Imu660ra_GetPitch() - pitch_offset;
    float roll  = My_Imu660ra_GetRoll()  - roll_offset;

    // 陀螺仪角速度 (imu660ra坐标系): gx对应Roll变化率, gy对应Pitch变化率
    float pitch_rate = imu660_gy;
    float roll_rate  = imu660_gx;

    // 目标角度为0 (水平)
    float pitch_error = 0.0f - pitch;
    float roll_error  = 0.0f - roll;

    // PD控制器: vx补偿前后倾斜, vy补偿左右倾斜
    float vx_target = (angle_pid_pitch.Kp * pitch_error + angle_pid_pitch.Kd * pitch_rate);
    float vy_target = -(angle_pid_roll.Kp  * roll_error  + angle_pid_roll.Kd  * roll_rate);

    // 速度限幅
    if (vx_target >  0.15f) vx_target =  0.15f;
    if (vx_target < -0.15f) vx_target = -0.15f;
    if (vy_target >  0.15f) vy_target =  0.15f;
    if (vy_target < -0.15f) vy_target = -0.15f;
    ...
    if (++cnt % 10 == 0) {
        printf("pitch=%.1f,roll=%.1f,gy=%.1f,gx=%.1f\n",
