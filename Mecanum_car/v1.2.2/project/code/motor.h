...
extern PID angle_pid_yaw;  // 角度外环PID
extern PID angle_pid_gyro; // 角速度内环PID (陀螺仪反馈)
extern PID angle_pid_pitch;  // 用于控制pitch（前后平衡）
extern PID angle_pid_roll;   // 用于控制roll（左右平衡）
extern float pitch_offset;
extern float roll_offset;
extern uint8_t balance_mode; //0表示正常模式，1为倒立摆模式
