...
#define UART_FREQ       CY_INITIAL_TARGET_PERI_FREQ                             // ?????????? ???80M
...
// ???????       uart_n         ?????
// ???????       uint8          0??????????    1??????????  
// ??????       uart_isr_mask(UART_0);                  
// ??????       ??????? ??????????
//-------------------------------------------------------------------------------------------------------------------
uint8 uart_isr_mask(uart_index_enum uart_n)
{
    uint8 isr_type = 1;                                                                     // ????????  0??????????   1??????????  
  
    if(Cy_SCB_GetRxInterruptMask(scb_module[uart_n]) & CY_SCB_UART_RX_NOT_EMPTY)            // ???????????
    {
        ...
        Cy_SCB_ClearRxInterrupt(scb_module[uart_n], CY_SCB_UART_RX_NOT_EMPTY);              // ???????????????
    }
    else if(Cy_SCB_GetTxInterruptMask(scb_module[uart_n]) & CY_SCB_UART_TX_DONE)            // ????0????????
    {           
        Cy_SCB_ClearTxInterrupt(scb_module[uart_n], CY_SCB_UART_TX_DONE);                   // ???????????????
...
//-------------------------------------------------------------------------------------------------------------------
// ???????       ??????????????
// ???????       uart_n          ???????? ???? zf_driver_uart.h ?? uart_index_enum ?????G??
// ???????       tx_pin          ???????????
// ???????       rx_pin          ???????????
// ???????       void
...
// ???????       dat             ???????????
// ???????       void
// ??????       uart_write_byte(UART_0, 0xA5);                    // ??????1??????????????0xA5???????????????????????????CPU??????????
// ??????
//-------------------------------------------------------------------------------------------------------------------
void uart_write_byte (uart_index_enum uart_n, const uint8 dat)
{
    // ??? FIFO ?§á?¦Ë (CYT4BB UART FIFO ?§³?16)
    while(Cy_SCB_GetNumInTxFifo(scb_module[uart_n]) >= 16);
    Cy_SCB_WriteTxFifo(scb_module[uart_n], dat);	
    // ???????????????????????
    while(Cy_SCB_GetNumInTxFifo(scb_module[uart_n]) > 0);
}
...
// ???????       len             ???????
// ???????       void
// ??????       uart_write_buffer(UART_0, &a[0], 5);
// ??????
//-------------------------------------------------------------------------------------------------------------------
...
// ???????       *str            ??????????????
// ???????       void
// ??????       uart_write_string(UART_0, "seekfree");
// ??????
//-------------------------------------------------------------------------------------------------------------------
...
// ???????       *dat            ???????????
// ???????       uint8           ?????????
// ??????       uint8 dat = uart_read_byte(UART_0);             // ???? UART_1 ????  ?????? dat ??????
// ??????
//-------------------------------------------------------------------------------------------------------------------
...
// ???????       *dat            ???????????
// ???????       uint8           1????????   0?????????????
// ??????       uint8 dat; uart_query_byte(UART_0, &dat);       // ???? UART_1 ????  ?????? dat ??????
// ??????
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  ???????      SBUS????????
//  ???????      uartn           SBUS????????(UART_0,UART_1,UART_2,UART_3)
//  ???????      baud            SBUS?????????
//  ???????      tx_pin          SBUS???????????
//  ???????      rx_pin          SBUS???????????
//  ???????      void
//  ??????      uart_sbus_init(UART_4, 100000, UART4_TX_P14_1, UART4_RX_P14_0);       // ?????????4 ??????100000 ??????????? 14_1 ??????????? P14_0
//  ??????
//-------------------------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------------------------
//  ???????      ????????
//  ???????      uartn           ????????(UART_0,UART_1,UART_2,UART_3)
//  ???????      baud            ?????????
//  ???????      tx_pin          ???????????
//  ???????      rx_pin          ???????????
//  ???????      void
//  ??????      uart_init(UART_0, 115200, UART0_TX_P00_1, UART0_RX_P00_0);       // ?????????0 ??????115200 ??????????? P0_1 ??????????? P0_0
//  ??????
//-------------------------------------------------------------------------------------------------------------------
