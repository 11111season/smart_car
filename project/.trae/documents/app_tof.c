...
static void print_result(RANGING_SENSOR_Result_t *Result)
{
  ...
  printf("=== TOF Data (Dist[mm] / Status) ===\r\n");

  for (int row = 0; row < Result->NumberOfZones / zones_per_line; row++)
  {
    printf("--- Row %d ---\r\n", row);
    for (int col = 0; col < (int)zones_per_line; col++)
    {
      int idx = row * zones_per_line + col;
      if (Result->ZoneResult[idx].NumberOfTargets > 0)
      {
        printf("  [%2d] %5lu / %lu\r\n", idx,
               (unsigned long)Result->ZoneResult[idx].Distance[0],
               (unsigned long)Result->ZoneResult[idx].Status[0]);
      }
      else
      {
        printf("  [%2d]     X / X\r\n", idx);
