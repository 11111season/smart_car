...
void small_driver_set_duty(int16 motor_duty_1, int16 motor_duty_2, int16 motor_duty_3, int16 motor_duty_4)
{
    ...
    motor_value.send_data_buffer[6] = (uint8)((motor_duty_4 & 0xFF00) >> 8);        // 拆分 电机3占空比 的高八位

    motor_value.send_data_buffer[7] = (uint8)(motor_duty_4 & 0x00FF);               // 拆分 电机3占空比 的低八位

    motor_value.send_data_buffer[8] = (uint8)((motor_duty_3 & 0xFF00) >> 8);        // 拆分 电机4占空比 的高八位

    motor_value.send_data_buffer[9] = (uint8)(motor_duty_3 & 0x00FF);               // 拆分 电机4占空比 的低八位
...
//          for(int i = 0; i < 4; i ++)
//        {
//            printf("speed %d:%d, ", i, motor_value.receive_speed_data[i]);
//        }
//        printf("\r\n");
//
//
