...
// 磁力计校准调试变量
float mag_raw_gauss[3];   // 校准前 (高斯)
float mag_calib[3];       // 校准后 (高斯)
...
void imu660rc_get_data(void)
{
    ...
    // 陀螺仪原始数据低通滤波，滤除高频噪声后再传入角速度环
     imu_data.gyro_x = PT1Filter_Apply(&filter_imu_gyro_x, imu_data.gyro_x);
     imu_data.gyro_y = PT1Filter_Apply(&filter_imu_gyro_y, imu_data.gyro_y);
     imu_data.gyro_z = PT1Filter_Apply(&filter_imu_gyro_z, imu_data.gyro_z);

     // TODO: 陷波滤波待实现（需添加NotchFilter_t类型及Init/Apply函数）
     // imu_data.gyro_x = NotchFilter_Apply(&notch_gyro_x, imu_data.gyro_x);
     // imu_data.gyro_y = NotchFilter_Apply(&notch_gyro_y, imu_data.gyro_y);
     // imu_data.gyro_z = NotchFilter_Apply(&notch_gyro_z, imu_data.gyro_z);

     // 低通滤波后的角速度存入PT1专用字段，供角速度环PID使用
     imu_data.gyro_x_pt1 = imu_data.gyro_x;
     imu_data.gyro_y_pt1 = imu_data.gyro_y;
     imu_data.gyro_z_pt1 = imu_data.gyro_z;
...
void Mahony_Update_NoMag(void)
{
    ...
    // 自适应Kp: 加速度偏离1g越大，Kp越小(信任陀螺仪独立积分)
    float acc_dev = fabsf(acc_norm - 9.8f) / 9.8f;
    float acc_factor = 1.0f - (acc_dev / MAHONY_ACC_DEV_THRESH);
    if (acc_factor > 1.0f) acc_factor = 1.0f;
    if (acc_factor < 0.0f) acc_factor = 0.0f;
    ...
    startup_timer += dt;
    float kp_max;
    if (startup_timer < MAHONY_STARTUP_TIME) {
        float ratio = startup_timer / MAHONY_STARTUP_TIME;
        kp_max = MAHONY_KP_STARTUP + (MAHONY_KP_NORMAL - MAHONY_KP_STARTUP) * ratio;
    } else {
        kp_max = MAHONY_KP_NORMAL;
    }

    const float Kp = MAHONY_KP_MIN + (kp_max - MAHONY_KP_MIN) * acc_factor;
...
void Mahony_Mag_Update(void)
{
    ...
    // 保存调试变量
    mag_raw_gauss[0] = mx_raw;
    mag_raw_gauss[1] = my_raw;
    mag_raw_gauss[2] = mz_raw;
    mag_calib[0] = mx_cal;
    mag_calib[1] = my_cal;
    mag_calib[2] = mz_cal;
    ...
    float kp_max;
    if (startup_timer_mag < MAHONY_STARTUP_TIME) {
        float ratio = startup_timer_mag / MAHONY_STARTUP_TIME;
        kp_max = MAHONY_KP_STARTUP + (MAHONY_KP_NORMAL - MAHONY_KP_STARTUP) * ratio;
    } else {
        kp_max = MAHONY_KP_NORMAL;
    }
...
void quarternion_to_rotation_matrix()
{
    ...
    // 体坐标系加速度 → 世界坐标系 (ar = R * ab - g_world)
    // 正确做法：三个轴都减去重力的投影分量
    float ax = imu_data.acc_x;
    float ay = imu_data.acc_y;
    float az = imu_data.acc_z;
    world_data.ax = rotation_matrix[0][0]*ax + rotation_matrix[0][1]*ay + rotation_matrix[0][2]*az - 9.8f * rotation_matrix[0][2];
    world_data.ay = rotation_matrix[1][0]*ax + rotation_matrix[1][1]*ay + rotation_matrix[1][2]*az - 9.8f * rotation_matrix[1][2];
    world_data.az = rotation_matrix[2][0]*ax + rotation_matrix[2][1]*ay + rotation_matrix[2][2]*az - 9.8f * rotation_matrix[2][2];
}