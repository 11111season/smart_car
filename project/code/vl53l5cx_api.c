		...
		VL53L5CX_Configuration		*p_dev)
{
	...
	if(status) { printf("[DBG] SW reboot seq failed: %d\r\n", status); goto exit; }

	/* Wait for sensor booted (several ms required to get sensor ready ) */
	status |= VL53L5CX_WrByte(&(p_dev->platform), 0x7fff, 0x00);
	status |= _vl53l5cx_poll_for_answer(p_dev, 1, 0, 0x06, 0xff, 1);
	if(status != (uint8_t)0){
		printf("[DBG] poll_for_answer(boot) failed: %d\r\n", status);
		goto exit;
	}
	...
	if(status != (uint8_t)0){
		printf("[DBG] poll_for_answer(FW access) failed: %d\r\n", status);
		goto exit;
	}
	status |= VL53L5CX_WrByte(&(p_dev->platform), 0x7fff, 0x00);
	...
	status |= _vl53l5cx_poll_for_answer(p_dev, 1, 0, 0x21, 0x10, 0x10);
	if(status != (uint8_t)0){
		printf("[DBG] poll_for_answer(FW check) failed: %d\r\n", status);
		goto exit;
	}
	printf("[DBG] FW check passed\r\n");
	...
	printf("[DBG] Xtalk sent\r\n");

	/* Send default configuration to VL53L5CX firmware */
	status |= VL53L5CX_WrMulti(&(p_dev->platform), 0x2c34,
		p_dev->default_configuration,
		sizeof(VL53L5CX_DEFAULT_CONFIGURATION));
	printf("[DBG] Default config sent\r\n");
	status |= _vl53l5cx_poll_for_answer(p_dev, 4, 1,
		VL53L5CX_UI_CMD_STATUS, 0xff, 0x03);
	if(status > VL53L5CX_STATUS_OK) { printf("[DBG] poll default config failed: %d\r\n", status); goto exit; }
	printf("[DBG] Default config poll done\r\n");
